    ...
    try {
      ...
      if (error) {
        if (error.message === "Invalid login credentials") {
          setError("Hibas e-mail cim vagy jelszo.");
        } else if (error.message === "Email not confirmed") {
          setError("Az e-mail cim meg nincs megerositeve. Ellenorizd a postafiokodat.");
        } else {
          setError("Hiba tortent a bejelentkezes soran. Probald ujra.");
        }
        return;
      }
        ...
        <div className="rounded-[var(--radius)] border border-border bg-card p-8 shadow-sm">
          ...
          {(error || urlError) && (
            <div className="mb-5 rounded-[var(--radius)] border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
              {error ||
                (urlError === "auth"
                  ? "Sikertelen bejelentkezes. Probald ujra."
                  : urlError === "confirm"
                  ? "Sikertelen e-mail megerosites. Probald ujra."
